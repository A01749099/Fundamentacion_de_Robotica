...
class YoloDetectorNode(Node):
    def __init__(self):
        ...
        self.model = YOLO('/home/daniel/Downloads/semaforo_largo.pt')

        self.bridge = CvBridge()
        self.subscription = self.create_subscription(
            CompressedImage,'/semaforo/image_raw/compressed',  # Tópico de entrada
            self.image_callback,
            10)
        
        self.image_publisher = self.create_publisher(
            Image,
            '/semaforo/image_detected',  # Tópico de salida de imagen
            10)

        self.detection_publisher = self.create_publisher(
            String,
            '/semaforo/detections',  # Tópico de salida de detecciones en texto
            10)

        self.get_logger().info("Semaforo Detector Node has started.")
    ...
    def image_callback(self, msg):
        try:
            ...
            results = self.model.predict(frame, conf=0.65, imgsz=640)
